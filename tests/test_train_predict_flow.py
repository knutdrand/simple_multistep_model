...
def test_full_cli_invocation_via_subprocess(tmp_path: Path):
    """Run train.py and predict.py as actual subprocesses with --config."""
    import subprocess
    ...
    subprocess.run(
        [
            ...
            str(model_path),
            str(TEST_DATA / f"historic_data_{PREDICTION_DATES[0]}.csv"),
            str(TEST_DATA / f"future_data_{PREDICTION_DATES[0]}.csv"),
            str(out_path),
            "--config",
            str(DEBUG_CONFIG),
        ],
    ...
    assert out_path.exists()
    assert pd.read_csv(out_path).shape[0] > 0
